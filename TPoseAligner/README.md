...
가장 간단한 방법은 **레포 루트의 통합 인스톨러**(`install_menus.py`)를
사용하는 것입니다. Retargeter, TPoseAligner 등 모든 도구의 메뉴를 한 번에
등록합니다. 자세한 내용은 [`../README.md`](../README.md) 의 "Quick
install" 섹션 참고.

TPoseAligner 만 단독으로 설치하고 싶다면 (다른 패키지가 없는 환경 등),
**이 패키지의** `install_menu.py` 를 startup 폴더에 두면 됩니다:

1. `TPoseAligner/install_menu.py` 를 다음 위치로 복사 (또는 symlink):
   - Windows: `Documents\MB\<버전>\config\Scripts\Startup\`
2. MotionBuilder 재시작
3. 상단 **"Tools"** 메뉴 하위에 **"TPose Align"** 서브메뉴가 추가됨
   (같은 컨벤션을 따르는 형제 도구가 있다면 모두 같은 "Tools" 메뉴를 공유)
4. **Tools → TPose Align → T-Pose Pair Align...** 등으로 접근

`install_menu.py` 는 `__file__` 기반으로 패키지 위치를 자동 탐지하므로
레포가 어디에 있든 동작합니다. "Tools" 부모 메뉴는 없을 때만 새로
만들기 때문에 여러 도구를 동시에 설치해도 메뉴가 중복되지 않습니다.
...
2. **Tools → TPose Align → T-Pose Pair Align...**
...
1. **Tools → TPose Align → Batch Retarget...**
